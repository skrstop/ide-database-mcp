...
- **实时日志输出开关**：工具窗口日志区域新增 Toggle Switch，可控制是否在 UI 上实时显示日志。关闭后搜索框、
  清空按钮等同步置灰，日志文件写入不受影响。默认关闭，兼容旧版本配置。
- **Settings 联动**：Settings 页面日志设置区域同步增加实时输出开关，与工具窗口状态自动同步。
- **ToggleSwitch 自定义组件**：新增原生滑动开关 UI 组件，替代传统 Checkbox，提供更直观的开关体验。

### Changed

- 工具窗口日志搜索区域的「上一个 / 下一个」按钮改为图标 + tooltip 样式，界面更简洁。
- 日志实时输出关闭时，跳过搜索高亮计算和文档监听回调，避免不必要的性能开销。
