...
- 将 Database 能力封装为 MCP 工具：
    - `db.listDataSources` — 列出已配置数据源
    - `db.listDatabases` — 列出某个数据源下的 catalog/schema
    - `db.executeQuery` — 执行只读 SQL
    - `db.executeDml` — 执行 DML（数据操作）语句
    - `db.executeDdl` — 执行 DDL（结构变更）语句
...
将 `YOUR_DS_NAME` 替换为 `db.listDataSources` 返回的数据源名称。
